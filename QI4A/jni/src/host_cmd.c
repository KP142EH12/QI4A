...
static void Host_Say(qboolean teamonly)
{
	...
	if (!fromServer && !teamonly)
		portable_snprintf (text, sizeof(text), "\001%s: %s", host_client->name, p1);
	else if (!fromServer && teamonly)
		portable_snprintf (text, sizeof(text), "\001(%s): %s", host_client->name, p1);
	else if(*(sv_adminnick.string))
		portable_snprintf (text, sizeof(text), "\001<%s> %s", sv_adminnick.string, p1);
	else
		portable_snprintf (text, sizeof(text), "\001<%s> %s", hostname.string, p1);
	p2 = text + strlen(text);
...
static void Host_Tell_f(void)
{
	...
	// note this uses the chat prefix \001
	if (!fromServer)
		portable_snprintf (text, sizeof(text), "\001%s tells you: ", host_client->name);
	else if(*(sv_adminnick.string))
		portable_snprintf (text, sizeof(text), "\001<%s tells you> ", sv_adminnick.string);
	else
		portable_snprintf (text, sizeof(text), "\001<%s tells you> ", hostname.string);
